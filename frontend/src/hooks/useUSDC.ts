  ...
  const refetchBalance = useCallback(async () => {
    ...
    try {
      ...
      if (provider && "getCode" in provider) {
        ...
        if (code === "0x") {
          throw new Error(
            `No contract found at address ${SEPOLIA_CONTRACTS.USDC}. This address may not be a valid contract or may be on a different network.`,
          );
        }
      }
  ...
  const approve = useCallback(
    async (amount: bigint) => {
      ...
      try {
        ...
        const tx = await approveAsync([SEPOLIA_CONTRACTS.FUND_POOL, amount.toString()]);
